...
#----------------------------------------------------------------------#
vgm_model <- vgm(psill = 1, model = "Exp", range = 30, nugget = 0)
...
summarize_results <- function(results_list) {
  avg_matrix <- Reduce("+", lapply(results_list, function(df) {
    df_num <- as.data.frame(lapply(df[, c("bias", "MAE", "MSE")], as.numeric))
...
for (l in l_values) {
  ...
  for (q in q_list) {
    ...
    for (j in 1:sim_count) {
      ...
      # Transform to Gumbel field
      z_gumbel <- mu - beta * log(-log(pnorm(sim@data[, j])))
      gumbel_data <- cbind(coords, z = z_gumbel)

      # Partition the spatial data
      nh <- 1:125; ns <- 126:525; nk <- 526:625
      ch <- gumbel_data[nh, c("x", "y")]; zh <- gumbel_data[nh, "z"]
      cs <- gumbel_data[ns, c("x", "y")]; zs <- gumbel_data[ns, "z"]
      ck <- gumbel_data[nk, c("x", "y")]; zk <- gumbel_data[nk, "z"]

      # Fit variogram to training data
      df_vg <- gumbel_data[nh, ]
      ...
      zk_bme <- bme_estimate(x = ck, ch, cs, zh, a, b, model_type, nugget,
                             sill, range)[, 1:2]
      zk_new <- new_bme_estimate(x = ck, ch, cs, zh, a, b, nq = 3)[, 1:2]
...
dir.create("Gumbel_Exp_results_unif", showWarnings = FALSE)
...
for (l_name in names(results_all)) {
  for (q_name in names(results_all[[l_name]])) {
    ...
    file_name <- paste0("Gumbel_Exp_results_unif/", l_name, "_", q_name, ".csv")
